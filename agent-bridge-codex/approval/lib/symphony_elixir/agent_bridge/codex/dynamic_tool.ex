defmodule SymphonyElixir.AgentBridge.Codex.DynamicTool do
  @moduledoc """
  ...
  defp tool_error_payload(tool_name, :missing_query) do
    %{"error" => %{"message" => "`#{tool_name}` requires a non-empty `query` string."}}
  end
  ...
  defp tool_error_payload(tool_name, :invalid_variables) do
    %{"error" => %{"message" => "`#{tool_name}.variables` must be a JSON object when provided."}}
  end
  ...
  defp tool_error_payload(@github_graphql_tool, reason) do
    %{"error" => %{"message" => "GitHub GraphQL tool execution failed.", "reason" => inspect(reason)}}
  end
  ...
  defp unsupported_tool_response(tool, supported_tools) do
  ...
  defp tracker_kind(opts), do: Keyword.get(opts, :tracker_kind)
end
