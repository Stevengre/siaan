defmodule SymphonyElixir.StatusDashboard do
  ...
  def render_offline_status do
    content =
      [
        colorize("╭─ SIAAN STATUS", @ansi_bold),
  ...
  defp format_project_link_lines do
    project_part =
      case Config.settings!().state.project_slug do
  ...
  defp running_table_header_row(running_event_width) do
    header =
      [
        ...
        format_cell("ISSUE/PHYS", @running_session_width),
  ...
  defp format_running_session_ids(running_entry) do
    logical = compact_session_id(Map.get(running_entry, :issue_session_id))
    physical = compact_session_id(Map.get(running_entry, :physical_session_id))
    "#{logical}/#{physical}"
  end
