defmodule SymphonyElixir.Install.Runner do
  ...
  defp finish_install(context, yes) do
    %{dry_run: dry_run, info: info} = context
  ...
  defp maybe_ensure_labels(%{security_config: %{setup: %{labels: false}}, info: info}) do
    info.("   ✓ Labels — disabled in .github/siaan-security.yml")
    :ok
  end

  defp maybe_ensure_labels(%{client: client, repo_ctx: repo_ctx, dry_run: dry_run, info: info}) do
    ensure_labels(client, repo_ctx, dry_run, info)
  end

  defp maybe_ensure_branch_protection(
         %{security_config: %{setup: %{branch_protection: false}}, info: info},
         _maintainers
       ) do
  ...
  defp maybe_ensure_branch_protection(
         %{client: client, repo_ctx: repo_ctx, branch: branch, dry_run: dry_run, info: info},
         maintainers
       ) do
    ensure_branch_protection(client, repo_ctx, branch, maintainers, dry_run, info)
  end

  defp maybe_resolve_default_branch(%{setup: %{branch_protection: false}}, _client, _repo_ctx, _opts),
    do: nil

  defp maybe_resolve_default_branch(_security_config, client, repo_ctx, opts) do
    resolve_default_branch(client, repo_ctx, opts)
  end
  ...
  defp branch_protection_payload(maintainers, current \\ nil) do
    ...
    %{
      ...
      "enforce_admins" => Map.get(normalized, "enforce_admins", false),
