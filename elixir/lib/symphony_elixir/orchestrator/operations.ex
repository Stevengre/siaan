defmodule SymphonyElixir.Orchestrator.Operations do
  @moduledoc false
  ...
  @spec handle_runner_down(State.t(), reference(), term()) :: State.t()
  def handle_runner_down(%State{running: running} = state, ref, reason) when is_reference(ref) do
  ...
  @spec update_runtime_info(State.t(), String.t(), map()) :: State.t()
  def update_runtime_info(%State{running: running} = state, issue_id, runtime_info)
  ...
  @spec handle_codex_update(State.t(), String.t(), map()) :: State.t()
  def handle_codex_update(
  ...
  def handle_codex_update(state, _issue_id, _update), do: state

  @spec handle_dispatch_complete(State.t(), String.t()) :: State.t()
  def handle_dispatch_complete(%State{running: running} = state, issue_id) do
    case Map.get(running, issue_id) do
      nil ->
        state
  ...
  def worker_slots_available_for_test(%State{} = state, issue, preferred_worker_host \\ nil) do
    worker_slots_available?(state, issue, preferred_worker_host)
  end
  ...
  @spec handle_retry_issue(State.t(), String.t(), non_neg_integer(), map()) ::
          {:noreply, State.t()}
  ...
  defp select_worker_host(%State{} = _state, %{project_runtime: runtime}, _preferred_worker_host)
       when runtime in ["local", :local],
       do: nil

  defp select_worker_host(%State{} = state, _issue, preferred_worker_host) do
  ...
  defp worker_slots_available?(%State{} = state, issue) do
    worker_slots_available?(state, issue, nil)
  end
